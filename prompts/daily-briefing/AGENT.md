...
**Duplicate-run guard:** if memory is available, immediately after reading `index.md`, also call `memory_read` with path `briefings/{{ DATE }}.html`. If that file exists (no error), the briefing has already been sent today. Output exactly:
"Briefing already sent for {{ TODAY }} — skipping duplicate run."
Then stop. Do not gather any data, do not call `gmail_send`.
